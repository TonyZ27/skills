...
1. **Explicit Selection (REQUIRED)**: Ask the user which session(s) they want to recap. Provide a list of recent conversation IDs and titles (8-10 items) if they haven't specified.
2. **Filtering**: Support keyword-based filtering (e.g., "all sessions about Project X").
    - If the user provides a keyword, list all matching sessions and ask for confirmation before proceeding.
3. The current conversation or thread.
4. User-provided transcripts or artifacts.

## Source Handling

**CRITICAL: NEVER proceed to full analysis without confirming the list of sessions with the user.**

### Step 0: Scope Selection & Confirmation

1. **Search & List**:
   - If the user provided a keyword or project name, use `grep_search` or `find_by_name` in `<appDataDir>/brain/` to find relevant logs/artifacts, OR list recent conversations and filter by title/summary.
   - Present a numbered list of candidates to the user.
2. **Ask for Intent**: Ask "Is this the correct set of sessions for the recap? Should I include/exclude anything specific?"
3. **Wait for Approval**: Proceed to Step 1 only after the user confirms.

### Step 1: Reconstruct & Analyze (After Confirmation)

1. Identify the session goal or goals.
2. Reconstruct the major phases...
