...
- **全局操作区 (Global Actions)**：
  ...
  - **导出功能 (Export to JSON)**：
    - **交互逻辑**：点击后提取当前过滤视图下的所有变量数据，并以 JSON 字符串形式发送至插件后台。
    - **状态覆盖 (State Matrix)**：
      - **Idle**：展示 `Download` 图标，背景跟随全局按钮规范。
      - **Loading**：按钮切换为 `RefreshCw` 旋转动画，且处于禁用状态以防重复点击。
      - **Success**：处理完成后，按钮切换为绿色 `Check` 图标并保持 2s，随后自动重置为 Idle 态。
      - **Error**：若导出失败（如内存溢出），上抛错误 Toast。
- **多层级导航布局**：
